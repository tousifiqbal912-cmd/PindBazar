...
// NOTE: No is_active, no display_order columns.
...
export default function BannersTab() {
  ...
  const resetForm = () => { setEditingId(null); setTitle(''); setSubtitle(''); setImageUrl(''); };
  ...
  const saveBanner = useMutation({
    mutationFn: async () => {
      ...
      // Only columns that exist: title, subtitle, image_url
      const payload = { title: title || null, subtitle: subtitle || null, image_url: imageUrl };
      if (editingId) {
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label>Banner Image * (Recommended 1920×800)</Label>
                {imageUrl ? (
                  ...
                  <div className="border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center text-muted-foreground aspect-[21/9] bg-muted/50 relative hover:border-primary transition-colors">
                    {isUploading ? (
                      <><Loader2 className="w-8 h-8 animate-spin mb-2" /><span>Uploading…</span></>
                    ) : (
                      <><Upload className="w-8 h-8 mb-2" /><span>Click to upload image</span></>
                    )}
              ...
              <div className="space-y-2">
                <Label>Title <span className="text-muted-foreground text-xs">(optional — shown over the image)</span></Label>
                <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Pure Village Goods" />
              </div>
              <div className="space-y-2">
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="text-center py-12 border border-dashed rounded-xl bg-muted/30 text-muted-foreground">
            No banners yet. Add one to show a slideshow on the homepage.
          </div>
        )}
      </div>
    </div>
